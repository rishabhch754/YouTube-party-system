...
const Room = () => {
  ...
  // ✅ Fetch role from backend on page load (ONCE)
  useEffect(() => {
    const fetchCurrentRole = async () => {
      try {
        const response = await fetch(`${apiUrl}/api/rooms/${roomId}`);
    ...
    fetchCurrentRole();
  }, [roomId, userId, apiUrl]);  // ✅ userRole REMOVED - no infinite loop
  ...
  useEffect(() => {
    ...
    wsService.current.connect(roomId, userId, username, {
      ...
      onUserLeft: (data) => {
        ...
        if (data.isHost) {
          addChatMessage('system', `⚠️ HOST ${data.username} has left the room!`);
          addChatMessage('system', `🏠 Redirecting to home page...`);
          setTimeout(() => {
